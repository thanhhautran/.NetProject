...
namespace WebShop.Areas.Admin.Data
{
    public class ChartDao
    {
        ...
        public int[] earningByQuarter()
        {
            ...
            {
                int tmp2 = tmp + 3;
                result[i] = (int)context.DonHang.Where(d => d.ngaygiaodich.Value.Month >= tmp && d.ngaygiaodich.Value.Month <= tmp2 
                                                            && d.ngaygiaodich.Value.Year == DateTime.Now.Year).Sum(s => s.giatridon);
                tmp += 3;
            }
            return result;
        }
        ...
        public int[] getProductSoldByMonth(string label)
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                var receiptDetail = context.Chitietdonhang.Where(s => s.sanpham.danhmuc.loaiSanPham == label 
                                                                    && s.donhang.ngaygiaodich.Value.Month == i
                                                                     && s.donhang.ngaygiaodich.Value.Year == DateTime.Now.Year).ToList();
        ...
        public int[] getProductSoldForHome(string label)
        {
            ...
            if (!label.Equals("Điện thoại"))
            {
                ...
                for (int i = 0; i < 12; i++)
                {
                    var receiptDetail = context.Chitietdonhang.Where(s => s.sanpham.danhmuc.loaiSanPham != "Điện Thoại"
                                                                        && s.donhang.ngaygiaodich.Value.Month == i
                                                                        && s.donhang.ngaygiaodich.Value.Year == DateTime.Now.Year).ToList();
